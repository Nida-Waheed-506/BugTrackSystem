...
const createBugFun = async (req, res, next) => {
  try {
    ...
    const bug = await createBug(project_id, QA_id, req?.file?.buffer, req.body);
...
const statusChangeFunc = async (req, res, next) => {
  try {
    const { project_id , bug_id: id } = req.params;
    const { status } = req.body;
    const {id: user_id} = req.user;
    const bug = await statusChange( project_id ,id, status ,user_id.toString());
